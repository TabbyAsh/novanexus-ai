import { PostgresEventStore, ChainVerificationResult } from '../postgres-store';
import { computeEventHash, generateId, nowTimestamp } from '@nova/shared';
import type { NovaEvent } from '@nova/shared';
...
describe('PostgresEventStore', () => {
  ...
  describe('emit - append-only enforcement', () => {
    ...
    it('should reject events with broken chain', async () => {
      ...
      const hash = computeEventHash(wrongPrevHash, payload, 'test.event', ts, 'USER', testActorId);
...
describe('Hash Chain Integrity', () => {
  it('should compute consistent hashes', () => {
    ...
    const hash1 = computeEventHash(prevHash, payload, type, ts, actorType, actorId);
    const hash2 = computeEventHash(prevHash, payload, type, ts, actorType, actorId);
  ...
  it('should detect payload tampering', () => {
    ...
    const originalHash = computeEventHash(prevHash, originalPayload, type, ts, actorType, actorId);
    const tamperedHash = computeEventHash(prevHash, tamperedPayload, type, ts, actorType, actorId);
  ...
  it('should detect timestamp tampering', () => {
    ...
    const hash1 = computeEventHash(prevHash, payload, type, ts1, actorType, actorId);
    const hash2 = computeEventHash(prevHash, payload, type, ts2, actorType, actorId);
  ...
  it('should detect actor tampering', () => {
    ...
    const hash1 = computeEventHash(prevHash, payload, type, ts, 'USER', 'user-123');
    const hash2 = computeEventHash(prevHash, payload, type, ts, 'USER', 'user-456');
  ...
  it('should detect chain break', () => {
    ...
    const ts1 = '2024-01-01T00:00:00.000Z';
    const event1Hash = computeEventHash(genesis, event1Payload, event1Type, ts1, 'USER', 'user-1');

    // Event 2 should reference event 1's hash
    const event2Payload = { seq: 2 };
    const event2Type = 'event.2';
    const ts2 = '2024-01-01T00:00:01.000Z';
    const correctEvent2Hash = computeEventHash(event1Hash, event2Payload, event2Type, ts2, 'USER', 'user-1');

    // If someone tries to use wrong prevHash
    const wrongPrevHash = 'wrong'.padEnd(64, '0');
    const tamperedEvent2Hash = computeEventHash(wrongPrevHash, event2Payload, event2Type, ts2, 'USER', 'user-1');
...
describe('verifyEventChain', () => {
  it('should verify valid chain', () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const hash = computeEventHash(prevHash, payload, type, ts, 'USER', 'user-1');
